...
# 💡 [핵심 해결책] 데이터 캐싱 (스마트 메모리 적용)
# 30초 동안 데이터를 기억하여 무분별한 구글 API 호출(429 에러)을 방지합니다.
...
def load_db_data():
    return (
        ...
        w_s.get_all_records(),
        w_r.get_all_records(),
        w_st.get_all_records(),
        w_u.get_all_records()
    )

# 💡 데이터가 변경(저장/수정/삭제)될 때만 메모리를 지우고 새로고침하는 함수
def apply_changes():
    load_db_data.clear() # 기억 지우기
    st.rerun()           # 새로고침
...
# 로그인할 때는 최신 계정 정보가 필요하므로 직접 읽어옵니다.
    ...
    with st.expander("🔄 매일 하는 데일리 업무 (루틴) 원클릭 등록", expanded=False):
        ...
        for i, r in enumerate(routine_data):
            if r.get('담당자') == u_name:
                ...
                if rc2.button("삭제", key=f"rdel_{r_idx}", disabled=is_locked):
